...
class OVODDetector:
    """Open Vocabulary Object Detection using Faster R-CNN and CLIP."""
    
    def __init__(self, device: str = None):
        """Initialize the OVOD detector with models."""
        self.device = device if device else ("cuda" if torch.cuda.is_available() else "cpu")
        print(f"Initializing OVOD Detector on {self.device}...")
        
        # Load Faster R-CNN
        self.frcnn = torchvision.models.detection.fasterrcnn_resnet50_fpn(pretrained=True)
        self.frcnn.eval()
    ...
    def generate_proposals(self, image: Image.Image, confidence_threshold: float = 0.5, 
                          max_proposals: int = 300):
        """Generate region proposals using Faster R-CNN."""
        ...
        with torch.no_grad():
            predictions = self.frcnn(image_tensor)
        
        # Filter by confidence threshold
        scores = predictions[0]['scores'].cpu().numpy()
        boxes = predictions[0]['boxes'].cpu().numpy()
        
        # Filter and limit proposals
    ...
    def detect(self, image: Image.Image, text_queries, 
        ...
        # RCNN inference (done once for all queries)
        if verbose:
            print("Generating region proposals...")
        rcnn_start = time.time()
        boxes, proposal_scores = self.generate_proposals(image, confidence_threshold, max_proposals)
    ...
    def detect_and_visualize(self, image_path: str, text_queries, 
        ...
        final_boxes, final_scores, final_similarities, final_classes = self.detect(
            ...
            max_proposals, top_k, iou_threshold, apply_nms, verbose=True, debug=debug
        )
...
def main():
    ...
    text_queries = ["cat", "dog"]  # Change to your query
